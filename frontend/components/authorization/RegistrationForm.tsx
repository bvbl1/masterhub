...
export default function RegistrationForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        setError(err.body.message ?? err.body.error ?? t("register.failed"));
      } else {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (!url?.trim()) {
        setError(t("register.googleStartFailed"));
        return;
      }
      ...
      if (err instanceof ApiError) {
        setError(
          err.body.message ?? err.body.error ?? t("register.googleFailed"),
        );
      } else {
        setError(t("register.genericError"));
      }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <motion.div
        ...
        animate="show"
      >
        <motion.div variants={fadeUp} className="mb-8 sm:mb-10 lg:mb-10">
        ...
        >
          ...
          <motion.div variants={fadeUp} className="mt-2">
            <label className="flex items-center gap-2.5 cursor-pointer select-none">
              ...
              />
              <span
                className={`mt-px flex h-5 w-5   border border-[#CBD5E1] items-center justify-center rounded transition-colors duration-150 `}
                aria-hidden
              >
                <CheckboxCheckIcon checked={agreed} />
              </span>
              <span className="text-xs sm:text-[12px] font-medium leading-relaxed pt-px text-[#475569]">
                {t("register.agreePrefix")}{" "}
                <span className="underline underline-offset-2 text-[#0F3DDE]">
                  {t("register.terms")}
                </span>
              </span>
            </label>
